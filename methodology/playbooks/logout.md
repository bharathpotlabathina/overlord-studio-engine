...
9. **Confirm to the Director:** what was committed, what was saved to memory, and the status of other repos in `_claude/repos.local` (clean, or which have uncommitted changes). Include one line: skills fired this session (tail the invocation log since session start) vs the non-negotiable expected set — report only, no judgment automated. Also run `node "{{PLUGIN}}/tools/reality-check.js" "{{VAULT}}"` and output the result verbatim under the heading `reality-check:`. BROKEN lines are findings for the next session or register candidates (`_claude/reality-check-ignore.txt` / `_claude/reality-aspirational.txt`) — never a blocker, report only. Sign off cleanly.
