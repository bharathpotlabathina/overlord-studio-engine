...
## Phase 5 — Constitution rewrite (proposed), I-82 verify, I-95 fix (2026-07-19)

**5.1:** Five Laws landed in the engine handbook (generic core); engine `doc-protocol.md` wiki line corrected to the Graveyard truth (sweep finding). `tools/checks/dedup-check.js` built + mutation-proven (seeded line 2→3 dupes, reverted) — and on its FIRST REAL RUN found 2 pre-existing verbatim codex↔handbook duplications (the exact I-84 disease). Their fix requires a codex edit: staged in the vault proposal `docs/superpowers/proposals/2026-07-19-five-laws-codex-landing.md`, **landing is the Director's act alone** via the source-of-truth gate. **The doctor is deliberately RED on `core-overlay-dedup` until that landing** — loud by design. Post-relocation firing tests re-run: 5/5 green. Paraphrase drift registered as a human-judgment check.
